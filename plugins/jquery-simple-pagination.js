	...
	*/
	...
	return this.each(function()
	{
		var container_id = '#' + $(this).attr('id'),
			items = $(this).find(settings.pagination_container).children(settings.pages_container),
		...
		function refresh_skip(page_number) 
		{
			if(page_number==1) {
			var skip_html = '<' + settings.navigation_element + ' href="#" class="' + settings.html_prefix + '-navigation-skip">' + settings.skip_content + '</' + settings.navigation_element + '>';
			return skip_html;
			}
			return '';
		}
		
		function refresh_close(page_number)
		{
			if (page_number == page_count) {
			var close_html = '<' + settings.navigation_element + ' href="#" class="' + settings.html_prefix + '-navigation-close">' + settings.close_content + '</' + settings.navigation_element + '>';
			return close_html;				
			}
			return '';
		}
		...
		function refresh_simple_pagination(page_number)
		{
			...
			if(settings.use_page_skip) {
				$(container_id + ' .' + settings.html_prefix + '-skip').html(refresh_skip(page_number));
			}
			
			if(settings.use_page_close) {
				$(container_id + ' .' + settings.html_prefix + '-close').html(refresh_close(page_number));
			}
			...
			if(settings.use_previous && (page_number > 1))
			{
